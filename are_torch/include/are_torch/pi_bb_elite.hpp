 ...
 */
class PiBBElite: public Learner{
public:
    typedef std::unique_ptr<PiBBElite> Ptr;
    typedef std::unique_ptr<const PiBBElite> ConstPtr;

    PiBBElite() : Learner(){}

    void init(int sample_size, int nbr_params, double sigma = 0.1, double inverse_lambda = 1, double decrement = 0.995, double lambda_decay = 1.0005, double elite_ratio = 0.5, bool maximisation = false);
...
private:
    ...
    double _lambda_decay;
    double _sigma;
    double _sigma_decay;
    int _sample_size;
    torch::Tensor _policy_params;
    torch::Tensor _exp_noise;
    torch::Tensor _elite_costs;
    torch::Tensor _samples;
    torch::Tensor _elite_samples;
