...
class SimulationFrame(Frame):
	def __init__(self,parent):
		...
		row += 1
		self.path = directory + '/' + fileRepository + '/' + 'SavedGenerations' + str(runNumber) +'.csv'
		...
		self.runNewHQ = Button(parent, text="Run New", command=self.clickRunHQ, height = buttonSize[1], width = buttonSize[0])
		...
		self.popQueueList.grid(column=3,row=1, columnspan = 3, rowspan = 6, sticky=E+W)
		...
		self.hybridQueue.grid(column=3,row=12, columnspan = 3, rowspan = 3, sticky=E+W)
	...
	def clickRunParallel(self):
		...
		if (os.name == 'nt'):
			subprocess.Popen([r""+directory+"/programming/v_repExtER_Minimal/x64/Debug/ERClient.exe",directory+"/files" ,"0" ,str(self.coreButton.get())])
		else:
...
class RealWorldFrame(Frame):
	def __init__(self,parent):
		...
		self.saveBPButton.grid(column=0, row=4);
