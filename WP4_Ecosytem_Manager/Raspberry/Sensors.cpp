/**
	@file Sensors.cpp
	@brief Implementation of methods for the sensors on the multi-sensor board.
	@author Mike Angus
*/
...
//Write bytes to control register
void ProximitySensor::init() {

	//Local storage for register bytes
	uint8_t highByte;
	uint8_t lowByte;

	//Configure control registers for proximity sensor
	//PS_CONF1 and PS_CONF2
	lowByte = 0x0E;  //Write 0x0E to PS_CONF1 (low byte) [1/40 DC, 8T Integration (Max), Power On)
	highByte = 0x08;  //Write 0x08 to PS_CONF2 (high byte) [16 bit, int. disable) 
	write16To(PS_CONF1_CONF2_REG, (uint16_t)highByte << 8 | lowByte);

	//PS_CONF3 and PS_MS
	lowByte = 0x00;  //Write 0x00 to PS_CONF3 (Low byte) [continuous mode]
	highByte = 0x07;  //Write 0x07 to PS_MS (high byte) [white channel enabled, 200ma current]
	write16To(PS_CONF3_MS_REG, (uint16_t)highByte << 8 | lowByte);
}

uint16_t ProximitySensor::read() {

	//Read the proximity data register and return the result
	return read16From(PS_DATA_REG);
}
...
void AmbientLightSensor::init() {
	//Write 0x00 to ALS_CONF [80ms int. time, interrupt disable, power on]
 	write8To(ALS_CONF_REG, 0x00);
}
//Reads the ambient light value
uint16_t AmbientLightSensor::readAmbient() {
	return read16From(ALS_DATA_REG);
}
//Reads the white value
uint16_t AmbientLightSensor::readWhite() {
	return read16From(WHITE_DATA_REG);
}


//***************************** ADC modules ********//

//Constructor for ambient ADC module
ADCmodule :: ADCmodule(uint8_t address) : I2CDevice(address)
{
...
//Reads the ADC value
uint16_t ADCmodule :: ADCread()
{
	//uint16_t temp = read8();
	//return (temp << 8) + read8();
	return read16From(0x9A);
...
ColorSensor :: ColorSensor(uint8_t address) : I2CDevice(address)
{
}
...
uint16_t ColorSensor :: readcolor(uint8_t ch)
{
	uint16_t color = 0;
	switch (ch)
	{
	case 0x00:
		color = read16From(0x08);
		break;
	case 0x01:
		color = read16From(0x09);
		break;
	case 0x02:
		color = read16From(0x0A);
		break;
	default:
		color = read16From(0x0B);
		break;
	}
	return color;
}
...
TOF::TOF(uint8_t address) : I2CDevice(address) {
	file_i2c = 0;
	iAddr = address;
}

int TOF::init(int iChan, int bLongRange)
{
	char filename[32];
...
//
unsigned short TOF::readReg16(unsigned char ucAddr)
{
...
//
unsigned char TOF::readReg(unsigned char ucAddr)
{
...
// write the register and then do multiple read
void TOF::readMulti(unsigned char ucAddr, unsigned char *pBuf, int iCount)
{
int rc;
...
void TOF::writeMulti(unsigned char ucAddr, unsigned char *pBuf, int iCount)
{
...
//
void TOF::writeReg16(unsigned char ucAddr, unsigned short usValue)
{
...
//
void TOF::writeReg(unsigned char ucAddr, unsigned char ucValue)
{
unsigned char ucTemp[2];
int rc;
...
//
void TOF::writeRegList(unsigned char *ucList)
{
...
int TOF::getSpadInfo(unsigned char *pCount, unsigned char *pTypeIsAperture)
{
int iTimeout;
unsigned char ucTemp;
...
uint16_t TOF::decodeTimeout(uint16_t reg_val)
{
...
uint32_t TOF::timeoutMclksToMicroseconds(uint16_t timeout_period_mclks, uint8_t vcsel_period_pclks)
{
...
uint32_t TOF::timeoutMicrosecondsToMclks(uint32_t timeout_period_us, uint8_t vcsel_period_pclks)
{
  uint32_t macro_period_ns = calcMacroPeriod(vcsel_period_pclks);
...
uint16_t TOF::encodeTimeout(uint16_t timeout_mclks)
{
  // format: "(LSByte * 2^MSByte) + 1"
...
void TOF::getSequenceStepTimeouts(uint8_t enables, SequenceStepTimeouts * timeouts)
{
...
int TOF::setVcselPulsePeriod(vcselPeriodType type, uint8_t period_pclks)
{
...
int TOF::setMeasurementTimingBudget(uint32_t budget_us)
{
...
uint32_t TOF::getMeasurementTimingBudget(void)
{
  uint8_t enables;
  SequenceStepTimeouts timeouts;
...
int TOF::performSingleRefCalibration(uint8_t vhv_init_byte)
{
int iTimeout;
...
//
int TOF::initSensor(int bLongRangeMode)
{
...
uint16_t TOF::readRangeContinuousMillimeters(void)
{
int iTimeout = 0;
uint16_t range;
...
int TOF::getModel(int *model, int *revision)
{
unsigned char ucTemp[2];
int i;
