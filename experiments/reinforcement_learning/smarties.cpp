...
void SmartInd::createMorphology(){
...
void SmartInd::createController(){
    ...
    //Create controller
    comm->setStateActionDims(nb_inputs,nb_outputs);
    ...
    comm->sendInitState(inputs);

}

void SmartInd::update(double delta_time){
    ...
    if( diff < 0.1){
        ...
        comm->sendState(inputs,instant_reward[0]);
...
//            std::cout << o << ";";
//        std::cout << std::endl;
        morphology->command(action);
    }
}

std::string SmartInd::to_string()
{
    std::stringstream sstream;
    boost::archive::text_oarchive oarch(sstream);
    oarch.register_type<SmartInd>();
...
void SmartInd::from_string(const std::string &str){
    std::stringstream sstream;
    sstream << str;
    boost::archive::text_iarchive iarch(sstream);
    iarch.register_type<SmartInd>();
...
void Smarties::init(){
    ...
    Individual::Ptr ind = std::make_shared<SmartInd>(morph_gen,ctrl_gen);
...
bool Smarties::update(const Environment::Ptr &env){
    Individual::Ptr ind = population[currentIndIndex];
    std::dynamic_pointer_cast<SmartInd>(ind)->set_trajectory(env->get_trajectory());
    return 0;
}

bool Smarties::is_finish(){
