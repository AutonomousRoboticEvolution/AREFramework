...
MoveObject::MoveObject(const settings::ParametersMapPtr& params)
{
    parameters = params;
...
std::vector<double> MoveObject::fitnessFunction(const Individual::Ptr &ind){
...
std::pair<int,int> MoveObject::real_coordinate_to_matrix_index(const std::vector<double> &pos){
...
float MoveObject::updateEnv(float simulationTime, const Morphology::Ptr &morph){
...
void MoveObject::build_tiled_floor(std::vector<int> &tiles_handles){
