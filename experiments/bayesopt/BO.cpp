...
obs_fct_t obs_fcts::objective = [](const BOIndividual::Ptr &ind) -> Eigen::VectorXd{

    std::vector<double> objs = ind->getObjectives();
...
void BO::init(){
    ...
    if(obs_type == settings::obsType::FINAL_POS)
        ...
        _compute_observation = obs_fcts::objective;
...
bool BO::update(const Environment::Ptr & env)
{
    ...
    if(instance_type == settings::INSTANCE_REGULAR || !simulator_side){
        ...
        if(generation > 0){
            double sigma =  std::dynamic_pointer_cast<BOLearner>(ind->get_learner())->get_sigma(std::dynamic_pointer_cast<NNParamGenome>(ind->get_ctrl_genome())->get_full_genome());
            Eigen::VectorXd mu = std::dynamic_pointer_cast<BOLearner>(ind->get_learner())->get_mu(std::dynamic_pointer_cast<NNParamGenome>(ind->get_ctrl_genome())->get_full_genome());
