...
/**
 * Return the observation of the closest sample from x
 */
template <typename Params>
struct ObsClosestSample : lb::mean::BaseMean<Params> {
    ObsClosestSample(size_t dim_out = 1) {}

    template <typename GP>
    Eigen::VectorXd operator()(const Eigen::VectorXd& x, const GP& gp) const
    {
        int dataset_size = gp.samples().size();
        Eigen::VectorXd D(dataset_size);
...
using gp_t = lb::model::GPOpt<Params>;//, kernel_t, mean_t, gp_opt_t>;
