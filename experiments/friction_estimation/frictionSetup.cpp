...
void FricInd::createMorphology(){
    ...
    std::dynamic_pointer_cast<sim::Morphology>(morphology)->createAtPosition(-0.05,0.05,0.24);
    int base_handle = std::dynamic_pointer_cast<sim::Morphology>(morphology)->getMainHandle();
    float orient[3];
...
float FrictionSetUp::updateEnv(float simulationTime, const sim::Morphology::Ptr &morph){
