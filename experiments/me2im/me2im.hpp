...
/**
 * @brief The ME2IMIndividual class
 */
class ME2IMIndividual : public Individual
{
public:
    ME2IMIndividual() : Individual(){}
    ME2IMIndividual(const Genome::Ptr& morph_gen,const EmptyGenome::Ptr& ctrl_gen)
        : Individual(morph_gen,ctrl_gen)
    {}
    ME2IMIndividual(const ME2IMIndividual& ind):
        Individual(ind){}


    Individual::Ptr clone() override {
        return std::make_shared<ME2IMIndividual>(*this);
    }
...
class ME2IM: public EA
{
public:
    typedef std::unique_ptr<ME2IM> Ptr;
    typedef std::unique_ptr<const ME2IM> ConstPtr;

    ME2IM() : EA(){}
    ME2IM(const misc::RandNum::Ptr& rn, const settings::ParametersMapPtr& param);
    ...
    const GridArchive<genome_t> &get_parent_pool() const {return parent_pool;}
...
private:
   ...
   GridArchive<genome_t> parent_pool;
