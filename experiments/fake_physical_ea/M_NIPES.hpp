...
class FakeRealMorph: public sim::Morphology_CPPNMatrix
{
    using CPPNMorph = sim::Morphology_CPPNMatrix;
public:
    FakeRealMorph(const settings::ParametersMapPtr &param): CPPNMorph(param){}

    void createAtPosition(float x, float y, float z) override{
        float building_time = settings::getParameter<settings::Float>(parameters,"#buildingTime").value;
        sleep(building_time);
        CPPNMorph::createAtPosition(x,y,z);
    }
};
