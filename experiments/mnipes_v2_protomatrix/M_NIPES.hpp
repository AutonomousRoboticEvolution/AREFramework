...
typedef struct learner_t{
    ...
    learner_t(const ProtomatrixGenome &mg): morph_genome(mg){}
    ProtomatrixGenome morph_genome;
...
typedef struct genome_t{
    ...
    genome_t(const ProtomatrixGenome &mg, const NNParamGenome &cg, const std::vector<double> &obj) :
...
typedef std::function<ProtomatrixGenome(const std::vector<genome_t>&)> selection_fct_t;
...
public:
    ...
    M_NIPESIndividual(const ProtomatrixGenome::Ptr& morph_gen,const Genome::Ptr& ctrl_gen)
