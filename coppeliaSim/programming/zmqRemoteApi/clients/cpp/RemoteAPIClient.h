...
public:
    RemoteAPIClient(const std::string host_ = "localhost", int rpcPort = 23000, int cntPort = -1, int verbose_ = -1);
    ~RemoteAPIClient();
    void connect();
    json call(const std::string &func, std::initializer_list<json> args,int timeout = 2500);
    json call(const std::string &func, const json &args = json(json_array_arg), int timeout = 2500);
