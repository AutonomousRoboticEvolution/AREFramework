...
if false then -- if not sim.isPluginLoaded('Bwf') then
    ...
    function sysCall_init()
        ...
        -- sim.writeCustomStringData(sim.getObject('..'),simBWF.modelTags.VISIONBOX,sim.packTable({version=1})) -- append the tag with data that just contains the version number
        model={}
