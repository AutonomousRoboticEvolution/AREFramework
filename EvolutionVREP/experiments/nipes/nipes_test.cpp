...
int main()
{
    ...
    while(best_fit > 0.5){
        ...
        for(are::Individual::Ptr& ind : pop){
            ...
            if(fit > 200) fit =  200;
            norm_fit = 1. - fit/200.;
            std::dynamic_pointer_cast<are::NN2Individual>(ind)->set_final_position({genome[0],genome[1],genome[2]}); //fake position because mandatory for novelty in this experiment
