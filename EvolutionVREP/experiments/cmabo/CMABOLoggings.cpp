...
void BOLog::saveLog(EA::Ptr &ea)
{
    ...
    Eigen::VectorXd obs = static_cast<const CMABO*>(ea.get())->getLastObs();
    Eigen::VectorXd spl = static_cast<const CMABO*>(ea.get())->getLastSpl();
...
void LearnerSerialLog::saveLog(EA::Ptr &ea){
    std::string repository = settings::getParameter<settings::String>(ea->get_parameters(),"#repository").value;
    int dataset_size = static_cast<const CMABO*>(ea.get())->get_learner()->dataset_size();
    std::stringstream sstream;
    sstream << Logging::log_folder << "/" << logFile << "_" << ea->get_generation() << "_" << dataset_size;
    limbo::serialize::TextArchive tarch(sstream.str());
    static_cast<const CMABO*>(ea.get())->get_learner()->model().save<limbo::serialize::TextArchive>(tarch);
}
